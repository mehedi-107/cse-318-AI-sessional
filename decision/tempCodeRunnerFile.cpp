...
double majorityError(const vector<string> &labels) {
    map<string,int> count;
    for (auto &lab: labels) count[lab]++;
    int maxCount = 0, n = labels.size();
    for (auto &p: count) {
        maxCount = max(maxCount, p.second);
    }
    double p_max = double(maxCount) / n;
    return 1.0 - p_max;
}
...
// Build decision tree with recursion (ID3).
Node* buildTree(const vector<vector<string>> &data, 
                const vector<string> &classes, 
                vector<int> attrIndices,
                const vector<bool> &isNumeric,
                const string &criterion, int depth, int maxDepth) 
{
    // Check termination
    ...
    if (attrIndices.empty() || (maxDepth>0 && depth >= maxDepth)) {
        // Find majority class
        map<string,int> count;
    ...
    chooseBestAttribute(data, classes, attrIndices, isNumeric, 
    ...
    for(int a: attrIndices) if (a != bestAttr) remaining.push_back(a);
...
// For the Adult dataset (no header) and Iris (with header).
vector<vector<string>> readCSV(const string &filename, bool hasHeader, bool dropFirstColumn=false) {
...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        ...
        cout << "Criterion = IG, IGR, or NWIG. maxDepth=0 for no pruning.\n";
        return 1;
    }
    string criterion = argv[1];
    int maxDepth = stoi(argv[2]);
    if (criterion != "IG" && criterion != "IGR" && criterion != "NWIG") {
        cerr << "Invalid criterion. Use IG, IGR, or NWIG.\n";
        return 1;
    }

    // --- Load datasets ---
    // Adult dataset (no header), drop any empty fields
    auto adultData = readCSV("Datasets/adult.data", false, false);
    // Iris dataset (CSV with header, drop ID column)
    auto irisData = readCSV("Datasets/Iris.csv", true, true);
    ...
    for (auto &dset: datasets) {
        ...
        for (int trial = 0; trial < 20; trial++) {
            ...
            Node *root = buildTree(trainData, trainLabels, attrIndices, isNumeric, criterion, 1, maxDepth);
